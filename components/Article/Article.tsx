    ...
    return (
        <Link href={'/article/' + id}>
            <Flex _hover={{ transform: "scaleX(1.005) translateZ(0)", backfaceVisibility: "hidden", filter: "blur(0)" }} filter="blur(0)" transitionDuration="0.5s" cursor="pointer" boxShadow={"lg"} my="8" borderRadius={["xl", "xl", "lg", "lg"]} overflow={"hidden"} direction={["column", "column", "row", "row"]} w="full">
                <Flex color="white" direction={"column"} p="8" pl={["8", "8", "8", "12"]} w={["100%", "100%", "35%", "35%"]} bg={bgTitle}>
                ...
                <Flex bg="white" mt={["-18", "-18", "0", "0"]} borderTopRadius={["2xl", "2xl", "0", "0"]} color="gray.700" direction={"column"} p="8" pt={["16", "16", "8", "8"]} w={["100%", "100%", "65%", "65%"]}>
                    <Flex alignItems={"center"} justifyContent={"space-between"} direction={"row"}>
                        <Text fontSize={["0.8em", "0.8em", "0.8em", "1.5em"]} fontWeight={"medium"}>ARTICLE {id}</Text>
                    ...
                    <Text fontWeight={"bold"} fontSize={["1.5em", "1.5em", "1.5em", "2em"]} isTruncated={true}>
                        {description}
                    </Text>
                </Flex>
            </Flex>
